...
GamePlayManager = {
  ...
  create: function () {
    ...
    for (var i = 0; i < AMOUNT_BOOBLES; i++) {
      ...
      var booble = game.add.sprite(
        xBooble,
        yBooble,
        "booble" + game.rnd.integerInRange(1, 2)
      );
      booble.vel = 0.1 + game.rnd.frac() * 2;
      booble.alpha = 0.9;
      booble.scale.setTo(.6 + game.rnd.frac());
    ...
    for (var i = 0; i < AMOUNT_DIAMONDS; i++) {
      ...
      while (
        this.isOverlapingOtherDiamond(i, rectCurrenDiamond) ||
  ...
  increaseScore: function () {
    ...
    if (this.amountDiamondsCaught >= AMOUNT_DIAMONDS) {
      game.time.events.remove(this.timerGameOver);
      this.endGame = true;
      this.showFinalMessage("Rescataste a todos");
    }
  },
